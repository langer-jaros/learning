...
void branch_and_bound(Problem &prob, Result &resu)
{
    ...
    while (++next_idx <= last_idx) {
        i = next_states[next_idx];
        lvl = (int)log2(i)+1;
        idx = n-lvl;
        if (idx > 0) {
            if (sts[i].v + prob.val_left[idx] > max_value) {
                next_states[++last_idx] = i*2;
                sts[i*2] = {idx, sts[i].w, sts[i].v, i};
                if ( sts[i].w+prob.items[idx].w <= prob.W) {
                    next_states[++last_idx] = i*2+1;
                    sts[i*2+1] = {idx, sts[i].w+prob.items[idx].w, sts[i].v+prob.items[idx].v, i};
                    if (sts[i].v+prob.items[idx].v > max_value) max_value = sts[i].v+prob.items[idx].v;
                }
            }
        } else {
            if (sts[i].w <= prob.W && sts[i].v > max.v)
...
bool read_problem(Problem &p)
{
    ...
    for (i = 0; i < p.n; i++) {
        ...
        p.val_left[i] = (i == 0)? v: p.val_left[i-1] + v;
    }
    return true;
}
